...
export class TodosService {
  ...
  addTodo(todo: Todo) {
    this._todos.update((v) => [...v, this.createStoredTodo(todo)]);
  }

  updateTodo(id: number, updatedTodo: Todo | StoredTodo) {
    if (id < 0) throw Error(`Wrong todo id to update: ${id}`);
    ...
    this._todos.update((todos) => {
      ...
      if (idx < 0) throw Error(`Todo to update not found: ${id}`);

      return todos.map((todo) =>
        todo.id === id ? this.createStoredTodo(updatedTodo) : todo,
      );
    });
  }
